...
#include "cache/active_cache_service/cache_client.h"
#include "cache/secondary_cache_local.h"
...
// Current the following isn't equivalent to OS_LINUX.
  ...
  static std::shared_ptr<SecondaryCache> NewFlashCache(uint64_t flash_size, uint64_t dram_size, bool enable_cache_filter, bool enable_admission_list) {
    if(FLAGS_use_local_flash_cache == false) {
      return nullptr;
    }
    return std::shared_ptr<SecondaryCache>(new SecondaryCacheLocal(flash_size, dram_size, enable_cache_filter, enable_admission_list));
  }

 public:
  Benchmark() :
        secondary_cache_(NewFlashCache(FLAGS_flash_cache_size, FLAGS_dram_cache_size, FLAGS_use_flash_cache_filter, FLAGS_enable_flash_admission_list)),
