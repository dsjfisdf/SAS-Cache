...
void CompactionJob::PrefetchInBackground(Slice& smallest_data, Slice& largest_data, int out_put_level) {
    ...
    std::shared_ptr<CacheClientLocal> client = std::shared_ptr<CacheClientLocal>(new CacheClientLocal(compact_->compaction->immutable_options()->secondary_cache));
    ...
    std::thread(&CacheClientLocal::KeyRangePrefetch, client, dbname, min_keys, max_keys, out_put_level).detach();
}

void CompactionJob::RecordInvalidFiles(std::vector<uint64_t>& files) {
    std::string dbname = dbname_;
    std::shared_ptr<CacheClientLocal> client = std::shared_ptr<CacheClientLocal>(new CacheClientLocal(compact_->compaction->immutable_options()->secondary_cache));
    client->RecordInvalidFiles(dbname, files);
}

void CompactionJob::EvictInvalidBlocks(std::vector<CacheKey>& blocks) {
    std::shared_ptr<CacheClientLocal> client = std::shared_ptr<CacheClientLocal>(new CacheClientLocal(compact_->compaction->immutable_options()->secondary_cache));
    std::thread(&CacheClientLocal::EvictInvalidBlocks, client, blocks).detach();
}
